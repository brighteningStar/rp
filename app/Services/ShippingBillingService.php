...
class ShippingBillingService extends ServiceAbstract
{
    ...
    public function getAll($columns = null)
    {
        ...
        if ( $keyword != '' ) {
            $shippingBillings = $this->model->select('shipping_billings.id as id', 'shipping_billings.name as name', 'shipping_billings.address as address', 'shipping_billings.address_type as address type')
