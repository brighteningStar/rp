<template>
    <section class="content">
        <div class="container-fluid">
            ...
            <table-vue uri="/forms/suppliers/get" title="Suppliers" delete-url="/forms/suppliers/id" cant-view="1"></table-vue>
        </div>
    </section>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            createItem(){
                this.form.post('/forms/suppliers')
            ...
            updateItem(){
                this.form.put('/forms/suppliers/'+this.editID)
                    .then(function (response) {
                        Event.$emit('reloadTable');
                        $("[data-dismiss=modal]").trigger({ type: "click" });
                    })
        ...
        mounted() {
            ...
            $('#modal-xl').on('hidden.bs.modal', function () {
                self.form.reset();
                self.title = "Create New Supplier";
                self.method = "create";
                this.editID = null;
            });
        },
        ...
        created() {
            Event.$on('editModal', (data) => {
                ...
                axios.get('/forms/suppliers/'+colorID)
                    .then(function (response) {
